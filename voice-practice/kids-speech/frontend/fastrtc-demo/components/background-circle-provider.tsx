...
export function BackgroundCircleProvider() {
    ...
    const handleConnected = useCallback(async () => {
        ...
        // Call the backend to reset greeting state when connection is established
        try {
            const response = await fetch('http://localhost:8000/greet');
            if (response.ok) {
                console.log('Greeting state reset successfully');
            } else {
                console.error('Failed to reset greeting state');
            }
        } catch (error) {
            console.error('Error calling /greet endpoint:', error);
        }
    }, []);
